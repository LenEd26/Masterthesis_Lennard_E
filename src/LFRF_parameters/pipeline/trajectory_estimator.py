...
class TuncaTrajectoryEstimator(AbstractTrajectoryEstimator):
    ...
    def estimate(
        ...
        if (
            os.path.exists(
                os.path.join(interim_data_path, "_tunca_estimation_left.json")
            )
            and os.path.exists(
                os.path.join(interim_data_path, "_tunca_estimation_right.json")
            )
            and not overwrite
        ):
            ...
            for foot in [("left", "LF"), ("right", "RF")]:
                trajectories[foot[0]] = pd.read_json(
                    os.path.join(interim_data_path, "_tunca_estimation_" + foot[0] + ".json")
                )

            # return {
            #     "left": pd.read_json(
            #         os.path.join(
            #             interim_data_path, "_tunca_estimation_left.json"
            #         )
            #     ),
            #     "right": pd.read_json(
            #         os.path.join(
            #             interim_data_path, "_tunca_estimation_right.json"
            #         )
            #     ),
            # }
        else:
            ...
            for foot in [("left", "LF"), ("right", "RF")]:
                ...
                trajectory.to_json(
                    os.path.join(
                        interim_data_path,
                        "_tunca_estimation_" + foot[0] + ".json",
                    )
                )
