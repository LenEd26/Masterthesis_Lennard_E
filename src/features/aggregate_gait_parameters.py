...
def aggregate_parameters(save_path, prefix="", select_strides=False, save=True):
    ...
    for side in ['left', 'right']:
        ...
        try:
            core_params[side] = pd.read_csv(os.path.join(save_path, prefix + side + "_foot_core_params_py_n.csv"), index_col=False)
    ...
    if save:
        aggregate_params["left"].to_csv(os.path.join(save_path,
                                                     prefix + "left_foot_aggregate_params_py_n.csv"), index=False)
        aggregate_params["right"].to_csv(os.path.join(save_path,
                                                      prefix + "right_foot_aggregate_params_py_n.csv"), index=False)
        aggregate_params_LR.to_csv(os.path.join(save_path,
                                              prefix + "aggregate_params_py_n.csv"), index=False)
