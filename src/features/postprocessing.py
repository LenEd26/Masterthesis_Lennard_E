...
def mark_interrupted_strides(df, sub, run, interrupt_path):
    ...
    for index, row in interrupt_df.iterrows():
        if np.logical_and(
                ...
                interrupt_df.loc[index, 'run'] == run[3:]  # remove prefix 'OG_' from column name
        ):
            # mark interrupted strides within the manually documented time interval
            true_idx = df[np.logical_and(
...
def select_stride_range(df, position, interval):
    ...
    df_subset = df.drop(df.index[
                            np.logical_or(df['is_outlier'] == 1, df['turning_interval'] == 1)
                        ])
...
def cut_by_stride(params_LF, params_RF, imu_path, save_path):
    ...
    # LF
    for idx, ic1, ic2, outlier, turning_step in zip(params_LF.stride_index,
                                                    params_LF.timestamps,
                                                    params_LF.ic_times,
                                                    params_LF.is_outlier,
                                                    params_LF.turning_step):
        # select the matching samples for that stride
        indizes = imu.data[(imu.data["timestamp"] >= ic1) & (imu.data["timestamp"] <= ic2)].index
    ...
    # RF
    for idx, ic1, ic2, outlier, turning_step in zip(params_RF.stride_index,
    ...
    for run in runs:
        for sub in sub_list:
            for foot in ['left', 'right']:  # comment out for cut_by_stride
                ...
                df_path = os.path.join(folder_path, foot + '_foot_core_params_py_n.csv')
                params_df = pd.read_csv(df_path)
                ### mark turning intervals
                params_df = mark_turning_interval(params_df, 2)  # add 'turning interval' column
...
if __name__ == "__main__":
    ...
    #         params_df.to_csv(os.path.join(folder_path, 'stable_gait_speeds', prefix + foot + '_foot_core_params_py_n.csv'), index=False)
    ...
    #         params_LF = pd.read_csv(os.path.join(folder_path, 'left' + '_foot_core_params_py_n.csv'))
    #         params_RF = pd.read_csv(os.path.join(folder_path, 'right' + '_foot_core_params_py_n.csv'))
    #
