...
reference_df_avg = reference_df[['stride_length_avg_left', 'clearance_avg_left', 'stride_time_avg_left',
       'swing_time_avg_left', 'stance_time_avg_left', 'stance_ratio_avg_left',
...
ref_norm = reference_df_avg.div(reference_df_avg).reset_index()
ref_norm.dropna(inplace=True)
print("ref NORM", ref_norm.describe())

visit_1_norm_ref = visit_1.div(reference_df_avg).reset_index()
visit_2_norm_ref = visit_2.div(reference_df_avg).reset_index()

#remove infinite and NAN values
visit_1_norm_ref.replace([np.inf, -np.inf], np.nan, inplace=True)
visit_2_norm_ref.replace([np.inf, -np.inf], np.nan, inplace=True)

visit_1_norm_ref.dropna(inplace=True)
visit_2_norm_ref.dropna(inplace=True)

visit_1_norm_ref.drop("index", inplace = True, axis = 1)
visit_2_norm_ref.drop("index", inplace = True, axis = 1)
ref_norm.drop("index", inplace = True, axis = 1)
...
v_2_norm_mean = visit_2_norm_ref.mean()
v_1_norm_mean = visit_1_norm_ref.mean()
print("Mean V2", v_2_norm_mean)
print("VISIT 2 LOC",visit_2_norm_ref.transpose())
...
visit_2_trans = visit_2_norm_ref.transpose()
visit_1_trans = visit_1_norm_ref.transpose()
reference_trans = ref_norm.transpose()
...
BLUE = "#2a475e"
fig.suptitle(
    "Average spatiotemporal features for both visits\nnormalized on the reference data - subject " + subject,
    x = 0.25,
    y = 0.97,
    ha="left",
    fontsize=10,
