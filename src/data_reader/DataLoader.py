...
class DataLoader:
    ...
    def load_GaitUp_data(self):
        ...
        self.data_df.drop(index=self.data_df.index[0], axis=0, inplace=True)  # drop first row with units
    ...
    def load_EXLs3_data(self):
        ...
        self.data_df = pd.read_fwf(os.path.join(folder_path, file_name),
                              header=3)
    ...
    def load_bonsai_data(self):
        ...
        data_df = raw_data_df.filter(['timestamp', 'accY', 'accX', 'accZ', 'gyrY', 'gyrX', 'gyrZ'], axis=1)
        data_df = data_df.rename(columns={'accY': 'AccX',
                                          'accX': 'AccY',
                                          'accZ': 'AccZ',
                                          'gyrY': 'GyrX',
                                          'gyrX': 'GyrY',
    ...
    def load_MM_data(self):
        ...
        acc_data_df = acc_data_df.filter(['epoc (ms)', 'elapsed (s)', 'x-axis (g)', 'y-axis (g)', 'z-axis (g)'], axis=1)
        gyro_data_df = gyro_data_df.filter(['x-axis (deg/s)', 'y-axis (deg/s)', 'z-axis (deg/s)'], axis=1)
        raw_data_df = pd.concat([acc_data_df, gyro_data_df], axis=1)
        raw_data_df = raw_data_df.dropna()  # match the number of data points
        raw_data_df['epoc (ms)'] = raw_data_df['epoc (ms)'] / 1000  # convert ms to s for the unix timestamp

        # adjust the xyz coordinates to match the script for EXLs3
        self.data_df = raw_data_df.rename(columns={
    ...
    def load_movi_data(self):
        ...
        acc_df = pd.read_csv(os.path.join(folder_path, foot_name, 'acc.csv'),
                             header=None, sep=';', names=['AccX', 'AccY', 'AccZ'])
        gyro_df = pd.read_csv(os.path.join(folder_path, foot_name, 'angularrate.csv'),
                              header=None, sep=';', names=['GyrX', 'GyrY', 'GyrZ'])
    ...
    def load_portabiles_data(self):
        ...
        # adjust the xyz coordinates to match the script for EXLs3
        data_df = raw_data_df.filter(['timestamp', 'acc_x', 'acc_y', 'acc_z', 'gyro_x', 'gyro_y', 'gyro_z'], axis=1)
        data_df = data_df.rename(columns={'acc_x': 'AccX',
                                          'acc_y': 'AccY',
                                          'acc_z': 'AccZ',
                                          'gyro_x': 'GyrX',
                                          'gyro_y': 'GyrY',
    ...
    def load_shimmer_data(self):
        ...
        # extract data columns
        raw_data_df = pd.read_csv(os.path.join(folder_path, file_name), skiprows=0, header=1)

        # adjust the xyz coordinates to match the analysis script
        data_df = raw_data_df.filter([foot_name + '_Timestamp_Unix_CAL',
                                      foot_name + '_Accel_WR_X_CAL',
                                      foot_name + '_Accel_WR_Y_CAL',
                                      foot_name + '_Accel_WR_Z_CAL',
                                      foot_name + '_Gyro_X_CAL',
                                      foot_name + '_Gyro_Y_CAL',
                                      foot_name + '_Gyro_Z_CAL'], axis=1)

        data_df = data_df.rename(columns={foot_name + '_Timestamp_Unix_CAL': 'timestamp',
        ...
        data_df = data_df.apply(pd.to_numeric)  # convert all columns of DataFrame to numbers
